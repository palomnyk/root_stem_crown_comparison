...
metabolites = read.table(file.path('.', ds,'lcPos.csv'), 
  sep=",", 
  header=TRUE, 
  row.names = 1, 
  check.names = FALSE)
...
lmMetadata = function(testingData, label){
  ...
  pdf(paste(ds, "HistogramMetaVs", label, "Lm.pdf", sep=""))
  hist(metaPval, 
  ...
  for( i in 1:nrow(dFrame)){
    ...
    plot( as.factor(as.character(metadata[as.character(dFrame$metaName[i]),])),
      ...
      col = brewer.pal(length(metadata[as.character(dFrame$metaName[i]),]),"Accent")
        ...
        print(paste(dFrame$indexName[i], unlist(testingData[dFrame$index[i],])))
        print(paste(dFrame$indexName[i], as.factor(as.character(metadata[as.character(dFrame$metaName[i]),]))))
  }
  dev.off()
  
  print('lmMetadata complete')
}#end of lmMetadata
bioActiveArr= c('Kavain')#lcPos is missing the other 2
