...
metabolites = read.table(file.path('.', '..','lcPos','lcPos.csv'), 
                         sep=",", 
                         header=TRUE, 
                         row.names = 1, 
                         check.names = FALSE)
...
plot.new()
legend('topright',
       legend = unique(as.factor(metadata['structural location',])),
       pt.bg = c(1,1,2),
       pch = c(24,21,22)[unique(as.numeric(as.factor(metadata['structural location',])))],
       cex = 1
)
# Commented code to verify legend
# legend('topright',
#        legend = as.factor(metadata['structural location',]),
#        pt.bg = as.factor(metadata['root vs stem',]),
#        pch = c(24,21,22)[as.numeric(as.factor(metadata['structural location',]))],
#        cex = 1
# )
dev.off()
...
lmeMetadata(t(myPCA), 'FullSamplesPca')
lmeMetadata(metabolites, 'FullSamples')
lmMetadata(metabolites, 'FullSamples')
pairwiseTtest(t(myPCA), 'tissue', 'FullSamplesPca')
pairwiseTtest(t(myPCA), 'structural location', 'FullSamplesPca')
pairwiseTtest(t(myPCA), 'root vs stem', 'FullSamplesPca')
pairwiseTtest(metabolites, 'root vs stem', 'FullSamplesPca')
pairwiseTtest(metabolites, 'tissue', 'FullSamplesMetabolites')
pairwiseTtest(metabolites, 'structural location', 'FullSamplesMetabolites')
print(summary(group_pca))