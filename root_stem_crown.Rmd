...
```{r}
#create PCA
group_pca = prcomp(t(metabolites), 
  center = TRUE,
  scale = TRUE)
...
plot(myPCA$PC1, myPCA$PC2,
  ...
  main = 'PCA with NA -> 0, log tranformed',
  col=l
)
legend('bottomright', legend = unique(l), 
  col = unique(l), 
  pch = 1, 
  cex=0.6
)
```
Each part clusters seperately, which is a good sign.


```{r}
plot(myPCA$PC1, myPCA$PC2,
  col=as.factor(meta_data['plant part',]))
legend('bottomright', 
  legend = unique(as.factor(meta_data['plant part',])), 
  col = unique(as.factor(meta_data['plant part',])), 
  pch = 1, 
  cex=0.6
  )
```
...
PCA with NA rows removed
```{r}
metabolitesNA = read.table(file.path('.','gc_data','gc_data_numeric.csv'), 
  sep=",", 
  header=TRUE, 
  row.names = 1, 
  check.names = FALSE)
...
```{r}
metabolitesNA = na.omit(metabolitesNA)
metabolitesNA = log2(metabolitesNA +1)
```


```{r}
#create PCA
group_pcaNA = prcomp(t(metabolitesNA), 
  center = TRUE,
  scale = TRUE)

#extract PCA matrix and convert to dataframe
myPcaNa = data.frame(group_pcaNA$x)
