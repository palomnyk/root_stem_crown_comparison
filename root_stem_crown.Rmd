...
dFrame <- data.frame(metaPval,metaPvalAdj,metaNames,metabos)
dFrame <- dFrame [order(dFrame$metaPvalAdj),]

write.table(dFrame, file=paste(ds, "MetaVsMetabolites.txt", sep = ''), row.names=FALSE, sep="\t")

write.table(failedLme, file=paste(ds, "MetaVsMetabolitesFails.txt", sep = ''), row.names=FALSE, sep="\t")
pdf(paste(ds, "MetaVsMetabolites.pdf", sep=""))
#par(mfrow=c(2,2))
