...
stripchart(metabolites,
  ...
  cex = 0.6,
  col = 'grey',
  xlab = metadata['experimental group',]
  )
plot.new()
...
colnames(expGroupPostHoc) = c('metabolite',"CRP - CNP","CWP - CNP","LR - CNP","SNP - CNP","SP - CNP","SWP - CNP","CWP - CRP","LR - CRP","SNP - CRP","SP - CRP","SWP - CRP","LR - CWP","SNP - CWP","SP - CWP","SWP - CWP","SNP - LR","SP - LR", "SWP - LR","SP - SNP","SWP - SNP","SWP - SP")
      ...
      tryCatch(
        ...
        if(myPval < 0.05){
          if(row.names(metadata)[i2] == 'plant part'){
            print('plant part')
            tukeyT = tukeysTest(myLm)
            plantPartPostHoc <<- rbind(plantPartPostHoc, data.frame(tukeyT))
          }
          if(row.names(metadata)[i2] == 'experimental group'){
            print('experimental group')
            tukeyT = tukeysTest(myLm)
            print(tukeyT)
            expGroupPostHoc <<- rbind(expGroupPostHoc, data.frame(tukeyT))
          }
        }
...
write.table(expGroupPartPostHoc, file=paste(ds, "ExpGroupPostHocLme.txt", sep = ''), row.names=FALSE, sep="\t")
...
Scratch space for making the columns of expGroupPostHoc
```{r}
cols = '"CRP - CNP" "CWP - CNP" "LR - CNP"  "SNP - CNP" "SP - CNP"  "SWP - CNP" "CWP - CRP" "LR - CRP"  "SNP - CRP" "SP - CRP"  "SWP - CRP" "LR - CWP"
 "SNP - CWP" "SP - CWP"  "SWP - CWP" "SNP - LR"  "SP - LR"   "SWP - LR"
 "SP - SNP"  "SWP - SNP" "SWP - SP"'
cols = gsub('\n', '', cols)
cols = gsub('  ', '","', cols)
cols = gsub('" "', '","', cols)
...
length(c('metabolites',"CRP - CNP","CWP - CNP","LR - CNP","SNP - CNP","SP - CNP","SWP - CNP","CWP - CRP","LR - CRP","SNP - CRP","SP - CRP","SWP - CRP","LR - CWP","SNP - CWP","SP - CWP","SWP - CWP","SNP - LR","SP - LR", "SWP - LR","SP - SNP","SWP - SNP","SWP - SP"))
```
