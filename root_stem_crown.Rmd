...
lmMetadata = function(testingData, label){
  ...
  for (i1 in 1:ncol(testingData)){
    for (i2 in 1:nrow(metadata)){
      myLm = lm( testingData[,i1] ~ as.factor(as.character(metadata[i2,])))
      pcas = c(pcas, i1)
...
```{r}
lmMetadata(myPCA, 'Pca')
```

Follow-up to previous block:
  pull just first 3 PCAs
```{r}
newFrame = subset(dFrame, pcas <= 3 )
write.table(newFrame, 
  file=paste(ds, "MetaVsPCAsLinearModTop3Pca.txt", sep = ''),
  row.names=FALSE, 
  sep="\t"
)
```
...
lmeMetadata = function(testingData, label){
  ...
  write.table(failedLme, file=paste(ds, "FailedMetaVs", label, ".txt", sep = ''), row.names=FALSE, sep="\t")
  
  pdf(paste(ds, "HistogramMetaVs", label, ".pdf", sep=""))
  hist(pvals, 
    main = paste('Histogram of pvalues for ', label),
