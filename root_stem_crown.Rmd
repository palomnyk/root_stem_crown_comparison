...
metabo = unlist(metabolitesNaO[i1,])
...
metabolitesNaO = na.omit(metabolites)

for (i1 in 1:nrow(metabolitesNaO)){
  for (i2 in 1:nrow(metadata)){
    if (row.names(metadata)[i2] != 'plant number'){
      #myLm = lm( unlist(metabolites[i1,]) ~ as.factor(as.character(metadata[i2,])))
      metabo = unlist(metabolitesNaO[i1,])
      metad = unlist(metadata[i2,])
      pn = unlist(metadata['plant number',])
      myD = data.frame(metabo, metad, pn)
      myLme = lme(metabo ~ metad, 
        ...
        data = myD)
      myAnova = anova(myLme)
      metaboliteNames = c(metaboliteNames, row.names(metabolitesNaO)[i1])
      metadataCatagories = c(metadataCatagories, row.names(metadata)[i2])
      pvals =  c(pvals, myAnova$"p-value"[2])
    }
  }
}
