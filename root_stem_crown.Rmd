      ...
      tryCatch(
        ...
        if(myPval < 0.05){
          if(row.names(metadata)[i2] == 'plant part'){
            ...
            index = names(plantPartPostHoc)
            
            myRow = myRow.sort(key=lambda (x,y): index.index(myRow))
            print(paste('after sort: ', myRow))
            myRow = c(myMetabo, myRow)
            print(myRow)
            plantPartPostHoc <<- rbind(plantPartPostHoc, myRow)
          }
...
How do I get the pvals out?
```{r}
if(!require(emmeans)){install.packages("emmeans")}
library(emmeans)

leastsquare = lsmeans(myLme, 
                      pairwise ~ metad, 
